...
def train_one_epoch(loader, model, args, criterion, optimizer):
    ...
    for idx, data in enumerate(loader):
        ...
        output = model(mlo=OM, cc=IM, us=TEM)
...
from MDL_IIA import MultiModalModel
parser.add_argument('--Description',
                    default='run_MDL-IIA。光镜+荧光+电镜。使用MDL-IIA模型进行多模态特征融合，OM=MG_mlo，IM=MG_cc，TEM=US，电镜固定一张。')
parser.add_argument('--savename', default='250309_MN分期-三模态全-前三期_MDL_IIA(光镜+荧光+电镜固一)')
parser.add_argument('--num_epochs', type=int,
                    default=40)
setproctitle.setproctitle("lkx_预计用至03/10/10：00.am")# 修改进程名称（放在最前面）
# *********************** 数据集相关 *********************** #
...
parser.add_argument('--CAM_img_path',
                    ...
                    type=str)
parser.add_argument('--reshape', default=[256, 256])
