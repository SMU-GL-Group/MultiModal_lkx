...
from MyModel.AGGN import AGGN_250106

parser.add_argument('--Description',
                    default='run_AGGN。光镜+荧光+电镜。使用AGGN多模态多尺度模型进行特征融合，电镜固定第一张。排除多阶段多尺度特征融合，探究其影响')
parser.add_argument('--savename', default='test')
parser.add_argument('--num_epochs', type=int,
                    default=40)
setproctitle.setproctitle("lkx_预计用至03/13/19：00.am")# 修改进程名称（放在最前面）
# *********************** 数据集相关 *********************** #
...
parser.add_argument('--batch_size', default=32, type=int, help='AGGN模型结构决定bs必须是8的倍数')
...
parser.add_argument('--CAM_img_path',
                    ...
                    type=str)
parser.add_argument('--reshape', default=[192,192])
...
def main():
    ...
    for k in range(0, args.kfold):
        ...
        train_loader = DataLoader(data_set, batch_size=args.batch_size, sampler=train_index[:64],num_workers=0,
                                  drop_last=True,pin_memory=False) # 锁页内存，加速数据传输读取
        test_loader = DataLoader(data_set, batch_size=args.batch_size, sampler=test_index[:64],num_workers=0,
    ...
    for i in range(0, args.kfold): # 从第i+1折开始
        ...
        # ======================= 模型设置 ======================= #
        model = AGGN_250106.Model(ch_in=3, ch_out=512, data_size=[4, 4, 192, 192], ch_out_2=512, args=args)
