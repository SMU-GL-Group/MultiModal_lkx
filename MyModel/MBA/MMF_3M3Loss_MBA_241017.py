# coding=utf-8
"""
使用Modality-based Attention[1]，对OM、IM、TEM拼接后的特征进行通道上的加权，赋予各模态不同权重。TEM不使用多示例。
...
class Model(nn.Module):
    ...
    def forward(self, TEM_tensor, OM_tensor, IM_tensor):
        ...
        x_TEM = self.TEM_encoder(TEM_tensor).permute(0, 3, 1, 2)  # [1,1024,7,7]
        TEM_pred = self.TEM_fc(x_TEM)
        TEM_output.append(TEM_pred)

        x_OM = self.OM_encoder(OM_tensor).permute(0, 3, 1, 2)
        OM_pred = self.OM_fc(x_OM)
        OM_output.append(OM_pred)

        x_IM = self.IM_encoder(IM_tensor).permute(0, 3, 1, 2)
