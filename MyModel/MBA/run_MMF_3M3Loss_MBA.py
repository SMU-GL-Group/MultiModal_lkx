...
from MyModel.MBA import MMF_3M3Loss_MBA_241017

parser.add_argument('--Description',
                    default='run_MMF_3M3Loss_MBA。模态拼接后送通道注意力(ModalityBasedAttn)，各模态视为通道。电镜固一。')
parser.add_argument('--savename', default='11-01_光镜（PASM）+荧光+电镜-301+270例_MMF_3M_MBA_241017(电镜固一,wLoss,bs=32续)')
...
def main():
    ...
    # ======================= 模型训练/测试 ======================= #
    for i in range(2, args.kfold): # 从第i+1折开始

        # ======================= 模型设置 ======================= #
        model = MMF_3M3Loss_MBA_241017.Model(args)
        model = model.to(args.device)
        model = nn.DataParallel(model, device_ids=[2,1])
        # if i == 0:
        #     print(model)
