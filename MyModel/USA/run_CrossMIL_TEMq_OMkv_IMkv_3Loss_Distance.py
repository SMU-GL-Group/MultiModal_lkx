...
from MyModel.USA import CrossMIL_TEMq_OMkv_IMkv_3Loss_250419


parser.add_argument('--Description',
                    default='run_CrossMIL_TEMq_OMkv_IMkv_3Loss_Distance。电镜q+光镜kv+荧光kv,wLoss。电镜加权光镜、荧光。电镜特征距离筛查与加权(权重归一化)。模态拼接。')
parser.add_argument('--savename', default='250419_光镜（PASM）+荧光+电镜-301+270例_CrossMIL_TEMq_OMkv_IMkv_3Loss_250419')
setproctitle.setproctitle("lkx_预计用至04/20/21：00.pm")  # 修改进程名称（放在最前面）
...
parser.add_argument('--stop_epochs', type=int, help='Skip remaining epochs if training has not improved after N epochs',
                    default=30)
...
def main():
    ...
    for k in range(1, args.kfold+1): # 从第k折开始
        ...
        # ======================= 模型设置 ======================= #
        model = CrossMIL_TEMq_OMkv_IMkv_3Loss_250419.Model(args)
        ...
        vis.Loss_Acc(args.n_epoch, args.savename, train_loss=train_loss_lst, test_loss=test_loss_lst,
    ...
    save_json(k_train_modal_loss_lst, main_name='k_train_modal_loss_lst', save_name=args.savename)
    save_json(k_epoch_avg_d_lst, main_name='k_epoch_avg_d_lst', save_name=args.savename)
    ...
    save_json(k_test_modal_loss_lst, main_name='k_test_modal_loss_lst', save_name=args.savename)
