...
def train_one_epoch(loader, model, args, criterion, optimizer):
    ...
    for idx, data in enumerate(loader):
        ...
        optimizer.zero_grad()
        total_loss = fusion_loss # 0.3 * OM_loss + 0.5 * IM_loss + 0.2 * TEM_loss +
        total_loss.backward()
        optimizer.step()
...
from MyModel.USA import USA_woSMIL_250122

parser.add_argument('--Description',
                    default='run_USA。电镜q+光镜kv+荧光kv。电镜加权光镜、荧光，模态拼接。电镜固定一张。无加权损失')
parser.add_argument('--savename', default='01-24_光镜（PASM）+荧光+电镜-301+270例_USA_woSMIL_250122(wo wLoss)')
...
def main():
    ...
    for i in range(0, args.kfold): # 从第i+1折开始
        ...
        # ======================= 模型设置 ======================= #
        model = USA_woSMIL_250122.Model(args)
        ...
        for row in range(3):
            ws.write(row + 1, 7, round(float(spe[row]), 3))
        # F1-score
        ws.write(0, 8, 'F1-score')
        for row in range(3):
            ws.write(row + 1, 8, round(float(f1_score(y_true=Actuals, y_pred=Preds, average=None)[row]), 3))
