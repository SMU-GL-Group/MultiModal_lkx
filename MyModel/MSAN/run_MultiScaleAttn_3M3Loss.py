#coding=utf-8
"""
使用MASN中的多尺度注意力模块Multi-Scale Attention SubNet[1]进行多模态训练。使用加权损失。
...
def train_one_epoch(loader, model, args, criterion, optimizer):
    ...
    for idx, data in enumerate(loader):
        ...
        # 最终预测结果以及各模态预测结果
        output, OM_output, IM_output, TEM_output = model(TEM_tensor=TEM, OM_tensor=OM, IM_tensor=IM)
...
from MyModel.MSAN import MultiScaleAttn_3M3Loss_240926
parser.add_argument('--Description',
                    default='run_MultiScaleAttn_3M3Loss。使用MultiScaleAttn模型进行多尺度特征提取，TEM固定第一张图像。无预训练，bs=4。')
parser.add_argument('--savename', default='250310_MN分期-三模态全-前三期_MultiScaleAttn_3M3Loss_240926(光镜多尺度,电镜固一,bs=4)')
parser.add_argument('--num_epochs', type=int,
                    default=40)
setproctitle.setproctitle("lkx_预计用至03/10/22：00 pm.")# 修改进程名称（放在最前面）
# *********************** 数据集相关 *********************** #
...
parser.add_argument('--modal', default='OM_+IM+TEM', type=str, help='所用模态：IM+TEM / OM+TEM / OM+IM 等')
...
def main():
    ...
    for k in range(0, args.kfold):
        ...
        train_loader = DataLoader(data_set, batch_size=args.batch_size, sampler=train_index, drop_last=True,num_workers=0)
        test_loader = DataLoader(data_set, batch_size=args.batch_size, sampler=test_index, drop_last=True,num_workers=0)
    ...
    for i in range(0, args.kfold):
        ...
        # ======================= 模型设置 ======================= #
        model = MultiScaleAttn_3M3Loss_240926.Model(args)
