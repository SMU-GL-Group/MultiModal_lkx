...
from MyModel.HiFuse import HiFuse
parser.add_argument('--Description',
                    default='run_HiFuse。电镜。使用HiFuse模型进行多尺度特征提取，作为单模态多尺度模型结果。')
parser.add_argument('--savename', default='10-17_光镜（PASM）+荧光+电镜-301+270例_HiFuse(电镜固一,bs=32)')
...
parser.add_argument('--modal', default='TEM', type=str, help='所用模态：IM+TEM / OM+TEM / OM+IM 等（OM-ori）')
...
def main():
    ...
    for i in range(0, args.kfold):
        ...
        # ======================= 模型设置 ======================= #
        model = HiFuse.HiFuse_Base(num_classes=args.num_cls)
