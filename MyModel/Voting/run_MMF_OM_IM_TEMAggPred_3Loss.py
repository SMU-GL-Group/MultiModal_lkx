...
def train_one_epoch(loader, model, args, criterion, optimizer):
    ...
    for idx, data in enumerate(loader):
        ...
        optimizer.zero_grad()
        total_loss = 1 * OM_loss + 1 * IM_loss + 1 * TEM_loss + fusion_loss
        total_loss.backward()
        optimizer.step()
...
from MyModel.Voting import MMF_OM_IM_TEMAggPred_240802

parser.add_argument('--Description',
                    default='MMF_OM_IM_TEMAggPred_3Loss。光镜+荧光+电镜。后期融合，各模态给出预测结果后相加。电镜每张图像预测结果相加作为电镜模态的预测结果。'
                            '每个模态的分类层改为自己写的MLP。各模态损失权重均为1。')
parser.add_argument('--savename', default='11-08_光镜（PASM）+荧光+电镜-301+270例_MMF_OM_IM_TEMAggPred_240802(bs=4)')
...
def main():
    ...
    for i in range(0, args.kfold): # 从第i+1折开始
        ...
        # ======================= 模型设置 ======================= #
        model = MMF_OM_IM_TEMAggPred_240802.Model(args)
