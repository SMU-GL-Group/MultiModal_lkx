...
def train_one_epoch(loader, model, args, criterion, optimizer):
    ...
    for idx, data in enumerate(loader):
        ...
        # 光镜+电镜
        output, OM_output, TEM_output = model(bags, OM_tensor=OM)
...
from MyModel.Voting import MMF_OM_TEMAggpred_241106

parser.add_argument('--Description',
                    default='run_MMF_OM_TEMAggPred。光镜+电镜。后期融合，各模态给出预测结果后相加。电镜每张图像预测结果相加作为电镜模态的预测结果。'
                            '每个模态的分类层改为自己写的MLP。无加权损失。')
parser.add_argument('--savename', default='11-08_MN分期-三模态全-前三期_MMF_OM_TEMAggpred_241106(光镜+电镜AggPred,bs=4,重复Ⅲ)')
...
parser.add_argument('--modal', default='OM_+TEM', type=str, help='所用模态：IM+TEM / OM+TEM / OM+IM')
...
def main():
    ...
    for i in range(0, args.kfold): # 从第i+1折开始
        ...
        # ======================= 模型设置 ======================= #
        model = MMF_OM_TEMAggpred_241106.Model(args)
