...
def train_one_epoch(loader, model, args, criterion, optimizer):
    ...
    for idx, data in enumerate(loader):
        ...
        batch_acc += torch.sum((torch.eq(pred, target))).item()
        total_sample_train += target.size(0)

    Train_acc = (100 * batch_acc / total_sample_train)
...
def Test(loader, model, args, criterion):
    ...
    with torch.no_grad():
        ...
        Test_acc = (100 * batch_acc / total_sample_test)
...
from MyModel.DiffEncoders import MMF_3M_ResNet_250107

parser.add_argument('--Description',
                    default='run_MMF_3M_ResNet。电镜+光镜+荧光。模态拼接。')
parser.add_argument('--savename', default='250301_MN分期-三模态全-前三期_MMF_3M_ResNet_250107')
...
def main():
    ...
    for k in range(0, args.kfold):
        ...
        train_loader = DataLoader(data_set, batch_size=args.batch_size, sampler=train_index,num_workers=0,
                                  drop_last=True,
                                  #collate_fn=TEM_dataset_collate
                                  )
    ...
    for i in range(0, args.kfold): # 从第i+1折开始
        ...
        # ======================= 模型设置 ======================= #
        model = MMF_3M_ResNet_250107.Model(args)
        ...
        # 冻结BN层的参数
        # for name, module in model.TEM_encoder.named_modules():
        #     if isinstance(module, torch.nn.BatchNorm2d):
        #         for param in module.parameters():
        #             param.requires_grad = False
        #
        # for name, module in model.OM_encoder.named_modules():
        #     if isinstance(module, torch.nn.BatchNorm2d):
        #         for param in module.parameters():
        #             param.requires_grad = False
        #
        # for name, module in model.IM_encoder.named_modules():
        #     if isinstance(module, torch.nn.BatchNorm2d):
        #         for param in module.parameters():
        #             param.requires_grad = False
