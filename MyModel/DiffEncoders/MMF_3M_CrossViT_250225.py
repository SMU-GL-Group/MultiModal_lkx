#coding=utf-8
"""
基于光镜、荧光、电镜的简单多模态融合。使用CrossViT编码器。
"""
import matplotlib.pyplot as plt
import torch.nn as nn
import torch
from torchvision import transforms
from MyModel.CrossViT.CrossViT import crossvit_base_224
...
class Model(nn.Module):
    def __init__(self, args):
        ...
        backbone = crossvit_base_224(pretrained=True)
        backbone.head = nn.ModuleList([nn.Linear(384, args.num_cls), nn.Linear(768, args.num_cls)])
    ...
    def forward(self, TEM_tensor, OM_tensor, IM_tensor):
        x_TEM = self.TEM_encoder(TEM_tensor)  # [bs,n_classes]
        x_OM = self.OM_encoder(OM_tensor)
        x_IM = self.IM_encoder(IM_tensor)

        # ********模态融合******** #
        stacked_tensor = torch.stack([x_TEM, x_OM , x_IM])
        out = torch.mean(stacked_tensor, dim=0)
