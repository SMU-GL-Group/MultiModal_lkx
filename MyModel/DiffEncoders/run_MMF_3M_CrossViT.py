...
from MyModel.DiffEncoders import MMF_3M_CrossViT_250225
parser.add_argument('--Description',
                    default='run_MMF_3M_CrossViT。光镜+荧光+电镜。简单特征融合，电镜固定一张。使用CrossViT作为编码器')
parser.add_argument('--savename',
                    default='250305_MN分期-三模态全-前三期_MMF_3M_CrossViT_250226(电镜固一,cat)')
parser.add_argument('--num_epochs', type=int,
                    default=40)
setproctitle.setproctitle("lkx_预计用至03/06/18：00.pm")# 修改进程名称（放在最前面）
# *********************** 数据集相关 *********************** #
...
def main():
    ...
    for i in range(0, args.kfold): # 从第i+1折开始
        ...
        # ======================= 模型设置 ======================= #
        model = MMF_3M_CrossViT_250225.Model(args)
