...
class Model(nn.Module):
    def __init__(self, args):
        ...
        backbone = nn.Sequential(*list(swin_b(weights=Swin_B_Weights).children())[:-4])  # 虽然初始化中有heads，但forward中不参与（方便参数对应上）
        # backbone.heads = zero()
    ...
    def forward(self, TEM_tensor, OM_tensor, IM_tensor):
        ...
        out = self.MLP(x_cat.permute(0,3,1,2))  # [bs,basic_dim*n]
