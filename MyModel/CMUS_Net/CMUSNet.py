...
class Model(nn.Module):
    def __init__(self, args):
        ...
        self.CrossAttn = CrossAttention(dim=basic_dim, num_heads=args.num_attn_heads)
    ...
    def forward(self, bag_tensor, OM_tensor, IM_tensor):
        ...
        for bs in range(len(bag_tuple)):  # 第bs个batch
            ...
            for bag in bag_tuple[bs]:  # 第bag个包
                ...
                for i in img_tuple:
                    # # 查看每张图像
                    # img = self.trans(torch.squeeze(i, dim=0))
                    # plt.imshow(img)
                    # plt.show()
                ...
                if len(TEMs) > 2:  # 至少有三个实例才能确定谁是距离较远的点
                    ...
                    valid_indices = [i for i, dist in enumerate(center_distances) if dist <= avg_distance * self.t]
