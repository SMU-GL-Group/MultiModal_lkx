...
def train_one_epoch(loader, model, args, criterion, optimizer):
    ...
    for idx, data in enumerate(loader):
        ...
        output = model(OM, IM, TEM, mask=None)
...
parser.add_argument('--Description',
                    default='run_mmFormer。使用mmFormer_cat模型进行多模态特征融合，电镜固定一张。使用脑分割的权重')
parser.add_argument('--savename', default='250307_MN分期-三模态全-前三期_mmFormer_cat(光镜+荧光+电镜固一,pretrained)')
parser.add_argument('--num_epochs', type=int,
                    default=40)
setproctitle.setproctitle("lkx_预计用至03/07/22：00.am")# 修改进程名称（放在最前面）
# *********************** 数据集相关 *********************** #
...
parser.add_argument('--CAM_img_path',
                    ...
                    type=str)
parser.add_argument('--reshape', default=[448, 448])
...
def main():
    ...
    for i in range(0, args.kfold): # 从第i+1折开始
        ...
        # ======================= 模型设置 ======================= #
        model = mmFormer_cat.Model(args=args)
        model.load_state_dict(torch.load(
            '/public/longkaixing/CrossModalScale/MyModel/mmFormer/pretrained weights/model_last.pth'),strict=False)
