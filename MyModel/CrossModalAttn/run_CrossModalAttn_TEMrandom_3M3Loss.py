...
def train_one_epoch(loader, model, args, criterion, optimizer):
    ...
    for idx, data in enumerate(loader):
        ...
        if 'TEM' in args.modal: # 含电镜
            # 荧光+电镜
            if len(args.modal.split('+')) == 2 and 'IM' in args.modal:
                output = model(TEM, OM_tensor=None, IM_tensor=IM)
            # 光镜+电镜
            elif len(args.modal.split('+')) == 2 and 'OM' in args.modal:
                output = model(TEM, OM_tensor=OM, IM_tensor=None)
            # 光镜+荧光+电镜
            elif len(args.modal.split('+')) == 3:
                output, OM_output, IM_output, TEM_output = model(TEM, OM_tensor=OM, IM_tensor=IM)
        # 光镜+荧光
        else:
            output = model(TEM_tensor=None, OM_tensor=OM, IM_tensor=IM)

        fusion_loss = criterion(output, target)
        OM_loss = criterion(OM_output, target)
        IM_loss = criterion(IM_output, target)
        TEM_loss = criterion(TEM_output, target)
...
from MyModel.CrossModalAttn import CrossAttn_TEMrandom_3M3Loss_241022
parser.add_argument('--Description',
                    default='run_CrossModalAttn_TEMrandom_3Loss。光镜+荧光+电镜。三模态相互交叉注意力，电镜固定一张，损失加权，多模态单尺度实验组。')
parser.add_argument('--savename', default='10-31_光镜（PASM）+荧光+电镜-301+270例_CrossAttn_TEMrandom_3M3Loss_241022(电镜固一,wLoss,bs=32续)')
...
def main():
    ...
    for k in range(0, args.kfold):
        ...
        train_loader = DataLoader(data_set, batch_size=args.batch_size, sampler=train_index,num_workers=4,
                                  drop_last=True,pin_memory=False) # 锁页内存，加速数据传输读取
        test_loader = DataLoader(data_set, batch_size=args.batch_size, sampler=test_index,num_workers=4,
    ...
    # ======================= 模型训练/测试 ======================= #
    for i in range(1, args.kfold): # 从第i+1折开始

        # ======================= 模型设置 ======================= #
        model = CrossAttn_TEMrandom_3M3Loss_241022.Model(args)
        ...
        for row in range(3):
            ws.write(row+1, 5, round(float(precision_score(y_true=Actuals, y_pred=Preds, average=None)[row]),3))
        # Recall
        ws.write(0,6, 'Recall')
        for row in range(3):
            ws.write(row+1, 6, round(float(recall_score(y_true=Actuals, y_pred=Preds, average=None)[row]),3))
        # F1-score
        ws.write(0, 7, 'F1-score')
        for row in range(3):
            ws.write(row+1, 7, round(float(f1_score(y_true=Actuals, y_pred=Preds, average=None)[row]), 3))
